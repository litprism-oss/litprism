...
async def _insert_article(db_session, project_id: str, **kwargs) -> Article:
    defaults = {
        "id": str(uuid.uuid4()),
...
async def test_dedup_keeps_unmatched(db_session, project_id):
    incoming = [
    ...
    result = await deduplicate(incoming, project_id, db_session)
    assert isinstance(result, DedupResult)
    assert len(result.new_articles) == 3
    assert len(result.duplicates) == 0


async def test_dedup_removes_doi_match(db_session, project_id):
    await _insert_article(db_session, project_id, title="Existing Article", doi="10.1234/test")
...
async def test_dedup_removes_pmid_match(db_session, project_id):
    await _insert_article(db_session, project_id, title="Existing Article", pmid="12345678")
...
async def test_dedup_fuzzy_title_match(db_session, project_id):
    await _insert_article(db_session, project_id, title="Effects of probiotics in Crohn disease")
...
async def test_dedup_within_batch_doi(db_session, project_id):
    incoming = [
...
async def test_dedup_normalises_doi_before_matching(db_session, project_id):
    await _insert_article(db_session, project_id, title="Existing", doi="10.1234/test")
...
async def test_dedup_writes_log_row(db_session, project_id):
    existing = await _insert_article(
        db_session, project_id, title="Logged Article", doi="10.1234/logged"
    )
    incoming = [ParsedArticle(title="Duplicate", doi="10.1234/logged")]
    await deduplicate(incoming, project_id, db_session)
    await db_session.commit()
