"""Pydantic models for litprism-europepmc.

The Article model is the shared contract between litprism-europepmc and downstream
...
class Article(BaseModel):
    ...
    Produced by EuropePMCClient.fetch() and consumed by litprism-screen.
    """

    # Identity — at least one of id/pmid must be present
    id: str  # source-specific internal id (Europe PMC article id)
    pmid: str | None = None
    doi: str | None = None
    source: Literal["europepmc"] = "europepmc"
    ...
    europepmc_source: str | None = None  # MED, PMC, PPR, etc.
...
class SearchResult(BaseModel):
    """Result of a Europe PMC search call — article ids only, not full records."""
